...
def getDataFromTxt(txt, with_landmark=True):
    ...
    for idx,line in enumerate(lines):
        ...
        assert(len(landmark) in [10, 196]), len(landmark)
        if len(landmark) == 10:
            label = -2
        else:
            label = -3
...
def GenLandmarkData(ftxt, net_type, size, output, argument=False, debug=False):
    ...
    for (imgPath, bbox, label, landmarkGt) in data:
        ...
        try:
            ...
            #litmit bbox
            gt_box = np.array([min(max(bbox.left,0),img_w),min(max(bbox.top,0),img_h),min(max(bbox.right,0),img_w),min(max(bbox.bottom,0),img_h)])
...
if __name__ == '__main__':
    ...
    if net == 'PNet':
        ...
        exit()
    #train_txt = './training_data/landmarks_5/trainImageList.txt'
    train_txt = './training_data/landmarks_70/ImageList_2.txt'
    output = './training_data/L48/landmark_test'
