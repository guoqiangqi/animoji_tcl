...
model_path = os.path.join(root_dir, '../models/LONet_wflw_98_L94_wingloss_finetune/LONet')
logs_dir = os.path.join(root_dir, '../models/logs/LONet_wflw_98_L94_wingloss_finetune')
# pre_model_path = os.path.join(root_dir, '../models/LONet_wflw_finetune_1/LONet-1600')
pre_model_path = os.path.join(root_dir, '../models/LONet_wflw_98_L94_wingloss/LONet-11')
...
cls_loss_op,bbox_loss_op,eye_eyebrow_loss_op,mouth_loss_op,multi_loss_op,L2_loss_op,accuracy_op, recall_op= L_O_Net(input_image, label, bbox_target,animoji_target,training=True)

loss = ratio_cls_loss*cls_loss_op + ratio_bbox_loss*bbox_loss_op \
     + ratio_eye_eyebrow_loss*eye_eyebrow_loss_op +ratio_mouth_loss*mouth_loss_op+ratio_multi_loss*multi_loss_op \
     + L2_loss_op
...
tf.summary.scalar('eye_eyebrow_loss',eye_eyebrow_loss_op)
...
try:
    ...
    for step in range(MAX_STEP):
        ...
        if (step+1) % display == 0:
            cls_loss, bbox_loss,eye_eyebrow_loss,mouth_loss,multi_loss,L2_loss,lr,acc,recall = \
                sess.run([cls_loss_op, bbox_loss_op,eye_eyebrow_loss_op,mouth_loss_op,multi_loss_op,L2_loss_op,lr_op,accuracy_op,recall_op],
                feed_dict={input_image: image_batch_array, label: label_batch_array, bbox_target: \
            ...
            print('{}: Step: {}, acc: {:.3}, recall: {:.3}, cls loss: {:.4}, bbox loss: {:.4}, eye_eyebrow loss: {:.4},mouth loss: {:.4},multi loss: {:.4}, L2 loss: {:.4}, lr:{:.4} '.format(
                datetime.now(), step+1, acc, recall, cls_loss, bbox_loss,  eye_eyebrow_loss,mouth_loss,multi_loss, L2_loss, lr))
        ...
        if i * BATCH_SIZE > num:
            ...
            cls_loss, bbox_loss,eye_eyebrow_loss,mouth_loss,multi_loss,L2_loss,lr,acc,recall = \
                ...
                bbox_batch_array,animoji_target: animoji_batch_array})
            print('{}: {}, Step: {}, acc: {:.3}, recall: {:.3}, cls loss: {:.4}, bbox loss: {:.4}, eye_eyebrow loss: {:.4},mouth loss: {:.4},multi loss: {:.4}, L2 loss: {:.4}, lr:{:.4} '.format( \
                'Save model epoch', epoch, step+1, acc, recall, cls_loss, bbox_loss, eye_eyebrow_loss,mouth_loss,multi_loss, L2_loss, lr))
            saver.save(sess, prefix, global_step=epoch)
            # saver.save(sess, prefix, global_step=step)
        writer.add_summary(summary,global_step=step)
