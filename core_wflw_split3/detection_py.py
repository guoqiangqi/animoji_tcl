...
class L_O_Detector(object):
    def __init__(self, net_factory, data_size, batch_size, model_path):
        ...
        with graph.as_default():
            self.image_op = tf.placeholder(tf.float32, shape=[batch_size, data_size, data_size, 3], name='input_image')            
            # self.cls_prob, self.bbox_pred, self.landmark_pred, self.animoji= net_factory(self.image_op, training=False)
            self.cls_prob, self.bbox_pred,self.eye_eyebrow,self.mouth,self.multi= net_factory(self.image_op, training=False)
            self.sess = tf.Session(
                config=tf.ConfigProto(allow_soft_placement=True, gpu_options=tf.GPUOptions(allow_growth=True)))
            saver = tf.train.Saver()
            model_dict = '/'.join(model_path.split('/')[:-1])
    ...
    def predict(self, databatch):
        ...
        for idx, data in enumerate(minibatch):
            ...
            if self.eye_eyebrow is None or self.mouth is None or self.multi is None :
                cls_prob, bbox_pred = self.sess.run([self.cls_prob, self.bbox_pred], feed_dict={self.image_op: data})
            else:
                cls_prob, bbox_pred,eye_eyebrow,mouth,multi = self.sess.run([self.cls_prob, self.bbox_pred,self.eye_eyebrow,self.mouth,self.multi], feed_dict={self.image_op: data})
                # animoji_pred_list.append(animoji[:real_size])
                # eye_pred_list.append(eye[:real_size])
                eye_eyebrow_pred_list.append(eye_eyebrow[:real_size])
        ...
        if self.eye_eyebrow is None or self.mouth is None or self.multi is None :
            # eye_pred_list = None
            eye_eyebrow_pred_list = None
            mouth_pred_list = None
            multi_pred_list = None
        else:
            # eye_pred_list = np.concatenate(eye_pred_list, axis=0)
            eye_eyebrow_pred_list = np.concatenate(eye_eyebrow_pred_list, axis=0)
            mouth_pred_list = np.concatenate(mouth_pred_list, axis=0)
            multi_pred_list = np.concatenate(multi_pred_list, axis=0)
        # return np.concatenate(cls_prob_list, axis=0), np.concatenate(bbox_pred_list, axis=0), np.concatenate(landmark_pred_list, axis=0),animoji_pred_list
        return np.concatenate(cls_prob_list, axis=0), np.concatenate(bbox_pred_list, axis=0),eye_eyebrow_pred_list,mouth_pred_list,multi_pred_list
...
class  detect_face(object):
    ...
    def predict(self,image):        
        ...
        _, oboxes_c,eye_eyebrow,mouth,multi,reg = self.onet.predict(image, rboxes_c)
        # if oboxes_c is not None:
            # print("oboxes_c")
            # print(oboxes_c.shape)
            # print(oboxes_c)
        # return pboxes_c, rboxes_c, oboxes_c, landmarks, animoji, reg
        return pboxes_c, rboxes_c, oboxes_c, eye_eyebrow,mouth,multi, reg
...
if __name__ == '__main__':
    ...
    model_path = [os.path.join(root_dir, '../models/PNet_1/PNet-60'), 
                  ...
                  os.path.join(root_dir, '../models/LONet_wflw_98_L94_wingloss_finetune/LONet-61'),
                  # os.path.join(root_dir, '../models/LONet_wflw_98_L94_wingloss_finetune/LONet-970'),
                  ]
    image_path = os.path.join(root_dir, '../image_test/')
    result_path=os.path.join(root_dir,'../result_data/LONet_wflw_98_L94_wingloss_finetune/LONet-61/expand0.3/')
    ...
    for f in glob.glob(os.path.join(image_path, '*.jpg')):
        image=cv2.imread(f)
        # pboxes,rboxes,boxes, landmarks, animojis,reg = facenet.predict(image.copy())
        pboxes,rboxes,boxes, eye_eyebrowes,mouthes,multis,reg = facenet.predict(image.copy())
        ...
        # if eyes is not None:
            # for eye in eyes:
                # for x,y in eye:
                    # cv2.circle(image, (int(x+0.5),int(y+0.5)), 1, (255,0,0)) 
        if eye_eyebrowes is not None:
            for eye_eyebrow in eye_eyebrowes:
                for x,y in eye_eyebrow:
                    cv2.circle(image, (int(x+0.5),int(y+0.5)), 1, (255,0,0)) 
