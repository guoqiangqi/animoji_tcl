...
def L_O_Net(inputs,label=None,bbox_target=None,animoji_target=None,training=True):
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        eyebrow_pred = slim.fully_connected(fc1,num_outputs=36,scope='eyebrow_fc',activation_fn=None)
        print(eyebrow_pred.get_shape())
        ...
        if training:
            ...
            cls_loss = cls_ohem(cls_prob,label)
            # bbox_loss = bbox_ohem(bbox_pred,bbox_target,label)
            bbox_loss = wing_loss(bbox_pred,bbox_target)
            
            accuracy, recall = cal_accuracy(cls_prob,label)
            # landmark_loss = landmark_ohem(landmark_pred, landmark_target,label)
            # animoji_loss = animoji_ohem(animoji_pred, animoji_target,label)
            # eye_loss = animoji_ohem(eye_pred, eye_target,label)
            # eyebrow_loss = animoji_ohem(eyebrow_pred, eyebrow_target,label)
            # mouth_loss = animoji_ohem(mouth_pred, mouth_target,label) 
            # multi_loss = animoji_ohem(multi_pred, multi_target,label)
            # eye_loss = wing_loss(eye_pred, eye_target)
            # eyebrow_loss = wing_loss(eyebrow_pred, eyebrow_target)
            # mouth_loss = wing_loss(mouth_pred, mouth_target) 
            # multi_loss = wing_loss(multi_pred, multi_target)
            
            #get contour result（17/33）
            # contour_result=tf.concat([multi_pred[:,0],multi_pred[:,1]],1)
            # for i in range(1,17):
                # contour_result=tf.concat([contour_result,multi_pred[:,4*i],multi_pred[:,4*i+1]],1)
            # #get eyebrow result（10/18）
            # eyeborw_result=tf.concat([eyeborw_pred[:,0],eyeborw_pred[:,1]],1)
            # for i in range(1,5):
                # eyeborw_result=tf.concat([eyeborw_result,eyebrow_pred[:,2*i],eyeborw_pred[:,2*i+1]],1)
            # for i in range(9,14):
                # eyeborw_result=tf.concat([eyeborw_result,eyebrow_pred[:,2*i],eyeborw_pred[:,2*i+1]],1)
            # #get nose result（9/9）
            # nose_result=tf.concat([multi_pred[:,2*33],multi_pred[:,2*33+1]],1)
            # for i in range(34,42):
                # nose_result=tf.concat([nose_result,multi_pred[:,2*i],multi_pred[:,2*i+1]],1)
            # #get eye result((2*6)/(2*8)+2)
            # eye_result=tf.concat([eye_pred[:,0],eye_pred[:,1]],1)
            # for i in range(2,7):
                # eye_result=tf.concat([eye_result,eye_pred[:,2*i],eye_pred[:,2*i+1]],1)
            # eye_result=tf.concat([eye_result,eye_pred[:,2*8],eye_pred[:,2*8+1]],1)
            # for i in range(10,15):
                # eye_result=tf.concat([eye_result,eye_pred[:,2*i],eye_pred[:,2*i+1]],1)
                
            contour_result=multi_pred[:,0:2]
            for i in range(1,17):
                contour_result=tf.concat([contour_result,multi_pred[:,4*i:4*i+2]],1)
            #get eyebrow result（10/18）
            eyebrow_result=eyebrow_pred[:,0:2]
            for i in range(1,5):
                eyebrow_result=tf.concat([eyebrow_result,eyebrow_pred[:,2*i:2*i+2]],1)
            for i in range(9,14):
                eyebrow_result=tf.concat([eyebrow_result,eyebrow_pred[:,2*i:2*i+2]],1)
            #get nose result（9/9）
            nose_result=multi_pred[:,2*33:2*33+2]
            for i in range(34,42):
                nose_result=tf.concat([nose_result,multi_pred[:,2*i:2*i+2]],1)
            #get eye result((2*6)/(2*8)+2)
            eye_result=eye_pred[:,0:2]
            for i in range(2,7):
                eye_result=tf.concat([eye_result,eye_pred[:,2*i:2*i+2]],1)
            eye_result=tf.concat([eye_result,eye_pred[:,2*8:2*8+2]],1)
            for i in range(10,15):
                eye_result=tf.concat([eye_result,eye_pred[:,2*i:2*i+2]],1)
                
            #get mouth result
            mouth_result=mouth_pred
            #get pupil result    
            pupil_result=tf.concat([eye_pred[:,34:36],eye_pred[:,32:34]],1)
                
            animoji_result=tf.concat([contour_result,eyebrow_result,nose_result,eye_result,mouth_result,pupil_result],1)
            animoji_loss=animoji_ohem(animoji_result, animoji_target,label)
            ...
            return cls_prob,bbox_pred,eye_pred,eyebrow_pred,mouth_pred,multi_pred
