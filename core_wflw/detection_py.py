...
class detect_L_onet(object):
    ...
    def predict(self, im, dets):
        ...
        cls_scores, reg,animoji = self.L_ONet.predict(cropped_ims)
        # print(reg.shape)
        # print(reg)
        #prob belongs to face
        cls_scores = cls_scores[:,1]        
        ...
        return boxes, boxes_c, animoji, reg
    def get_params(self):
...
class  detect_face(object):
    ...
    def predict(self,image):        
        ...
        _, oboxes_c,animoji,reg = self.onet.predict(image, rboxes_c)
        # if oboxes_c is not None:
            # print("oboxes_c")
            # print(oboxes_c.shape)
            # print(oboxes_c)
        # return pboxes_c, rboxes_c, oboxes_c, landmarks, animoji, reg
        return pboxes_c, rboxes_c, oboxes_c, animoji, reg
...
if __name__ == '__main__':
    ...
    model_path = [os.path.join(root_dir, '../models/PNet_1/PNet-60'), 
                  ...
                  os.path.join(root_dir, '../models/LONet_wflw_finetune_2/LONet-637'),
                  # os.path.join(root_dir, '../models/LONet_0805/LONet-340'),
                  ]
    image_path = os.path.join(root_dir, '../images_xuhua/')
    result_path=os.path.join(root_dir,'../xuhua_result_data/LONet_wflw_finetune_2/LONet-637/expand0.0/')
    ...
    for f in glob.glob(os.path.join(image_path, '*.jpg')):
        image=cv2.imread(f)
        # pboxes,rboxes,boxes, landmarks, animojis,reg = facenet.predict(image.copy())
        pboxes,rboxes,boxes, animojis,reg = facenet.predict(image.copy())
