...
class Detector(object):
    def __init__(self, net_factory, data_size, batch_size, model_path):
        ...
        with graph.as_default():
            ...
            self.cls_prob, self.bbox_pred,self.animoji= net_factory(self.image_op, training=False)
    ...
    def predict(self, databatch):
        ...
        for idx, data in enumerate(minibatch):
            ...
            if self.animoji is None:
                ...
                cls_prob, bbox_pred,animoji = self.sess.run([self.cls_prob, self.bbox_pred,self.animoji], feed_dict={self.image_op: data})
                animoji_pred_list.append(animoji[:real_size])
            cls_prob_list.append(cls_prob[:real_size])
            bbox_pred_list.append(bbox_pred[:real_size])
            # landmark_pred_list.append(landmark_pred[:real_size])
        ...
        return np.concatenate(cls_prob_list, axis=0), np.concatenate(bbox_pred_list, axis=0),animoji_pred_list
...
class L_O_Detector(object):
    def __init__(self, net_factory, data_size, batch_size, model_path):
        ...
        with graph.as_default():
            self.image_op = tf.placeholder(tf.float32, shape=[batch_size, data_size, data_size, 3], name='input_image')            
            # self.cls_prob, self.bbox_pred, self.landmark_pred, self.animoji= net_factory(self.image_op, training=False)
            self.cls_prob, self.bbox_pred,self.eye,self.eyebrow,self.mouth,self.multi= net_factory(self.image_op, training=False)
            self.sess = tf.Session(
                config=tf.ConfigProto(allow_soft_placement=True, gpu_options=tf.GPUOptions(allow_growth=True)))
            saver = tf.train.Saver()
            model_dict = '/'.join(model_path.split('/')[:-1])
    ...
    def predict(self, databatch):
        ...
        for idx, data in enumerate(minibatch):
            ...
            if self.eye is None or self.eyebrow is None or self.mouth is None or self.multi is None :
                cls_prob, bbox_pred = self.sess.run([self.cls_prob, self.bbox_pred], feed_dict={self.image_op: data})
            else:
                cls_prob, bbox_pred,eye,eyebrow,mouth,multi = self.sess.run([self.cls_prob, self.bbox_pred,self.eye,self.eyebrow,self.mouth,self.multi], feed_dict={self.image_op: data})
                # animoji_pred_list.append(animoji[:real_size])
                eye_pred_list.append(eye[:real_size])
                eyebrow_pred_list.append(eyebrow[:real_size])
                mouth_pred_list.append(mouth[:real_size])
                multi_pred_list.append(multi[:real_size])
            cls_prob_list.append(cls_prob[:real_size])
            bbox_pred_list.append(bbox_pred[:real_size])
            # landmark_pred_list.append(landmark_pred[:real_size])
        ...
        if self.eye is None or self.eyebrow is None or self.mouth is None or self.multi is None :
            eye_pred_list = None
            eyebrow_pred_list = None
            mouth_pred_list = None
            multi_pred_list = None
        else:
            eye_pred_list = np.concatenate(eye_pred_list, axis=0)
            eyebrow_pred_list = np.concatenate(eyebrow_pred_list, axis=0)
            mouth_pred_list = np.concatenate(mouth_pred_list, axis=0)
            multi_pred_list = np.concatenate(multi_pred_list, axis=0)
        # return np.concatenate(cls_prob_list, axis=0), np.concatenate(bbox_pred_list, axis=0), np.concatenate(landmark_pred_list, axis=0),animoji_pred_list
        return np.concatenate(cls_prob_list, axis=0), np.concatenate(bbox_pred_list, axis=0),eye_pred_list,eyebrow_pred_list,mouth_pred_list,multi_pred_list
...
class detect_rnet(object):
    ...
    def predict(self, im, dets):
        ...
        cls_scores, reg, _ = self.RNet.predict(cropped_ims)
...
class detect_L_onet(object):
    def __init__(self, model_path, threshold=0.7, batch_size=16):
        ...
        self.threshold = threshold
        self.expand = 0.3
        self.L_ONet = L_O_Detector(L_O_Net, self.net_size, batch_size, model_path)
    ...
    def predict(self, im, dets):
        ...
        cls_scores, reg,eye,eyebrow,mouth,multi = self.L_ONet.predict(cropped_ims)
        # print(reg.shape)
        # print(reg)
        #prob belongs to face
        cls_scores = cls_scores[:,1]        
        ...
        eyebrow = eyebrow.reshape(-1,18,2)*wh[:,None,:] + xy[:,None,:]       
        ...
        eye = eye[keep]
        eyebrow = eyebrow[keep]
        mouth = mouth[keep]
        multi = multi[keep]
        # return boxes, boxes_c, landmark, animoji, reg
        return boxes, boxes_c, eye,eyebrow,mouth,multi, reg
    def get_params(self):
...
class  detect_face(object):
    ...
    def predict(self,image):        
        ...
        _, rboxes_c,_ = self.rnet.predict(image, pboxes_c)
        # print('rboxes_c')
        ...
        _, oboxes_c,eye,eyebrow,mouth,multi,reg = self.onet.predict(image, rboxes_c)
        # if oboxes_c is not None:
            # print("oboxes_c")
            # print(oboxes_c.shape)
            # print(oboxes_c)
        # return pboxes_c, rboxes_c, oboxes_c, landmarks, animoji, reg
        return pboxes_c, rboxes_c, oboxes_c, eye,eyebrow,mouth,multi, reg
...
if __name__ == '__main__':
    ...
    model_path = [os.path.join(root_dir, '../models/PNet_1/PNet-60'), 
                  ...
                  os.path.join(root_dir, '../models/LONet_wflw_98_L94_finetune/LONet-11'),
                  # os.path.join(root_dir, '../models/LONet_wflw_98_L94_finetune/LONet-937'),
                  ]
    image_path = os.path.join(root_dir, '../image_test/')
    result_path=os.path.join(root_dir,'../result_data/LONet_wflw_98_L94_finetune/LONet-11/expand0.3/')
    ...
    #save_model
    facenet.save_model_bin(os.path.join(root_dir, '../numpys/face_model.bin'))    
    ...
    for f in glob.glob(os.path.join(image_path, '*.jpg')):
        image=cv2.imread(f)
        # pboxes,rboxes,boxes, landmarks, animojis,reg = facenet.predict(image.copy())
        pboxes,rboxes,boxes, eyes,eyebrowes,mouthes,multis,reg = facenet.predict(image.copy())
        ...
        if eyes is not None:
            for eye in eyes:
                for x,y in eye:
                    cv2.circle(image, (int(x+0.5),int(y+0.5)), 1, (255,0,0)) 
        if eyebrowes is not None:
            for eyebrow in eyebrowes:
                for x,y in eyebrow:
                    cv2.circle(image, (int(x+0.5),int(y+0.5)), 1, (255,0,0)) 
        ...
        p,g=os.path.split(f)
        # cv2.imshow(g,image)
        cv2.imwrite(result_path+g,image)
    k =cv2.waitKey(0)
    if k==ord('q'):
