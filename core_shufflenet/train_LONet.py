...
from mtcnn_shufflenet import O_Net,L_O_Net,read_multi_tfrecords,train_model,random_flip_images

pre_model_path = None
root_dir = os.path.dirname(os.path.realpath(__file__)) 
base_dir = os.path.join(root_dir, '../data/training_data/L224_wflw/imglists/LONet')
model_path = os.path.join(root_dir, '../models/LONet_shufflenet_L224/LONet')
logs_dir = os.path.join(root_dir, '../models/logs/LONet_shufflenet_L224')
# pre_model_path = os.path.join(root_dir, '../models/LONet_shufflenet_moreL2/LONet-404')
# pre_model_path = os.path.join(root_dir, '../models/LONet_wflw_finetune_1/LONet-569')
...
pos_radio = 1.0/7;part_radio = 1.0/7;neg_radio=3.0/7;animoji_radio=2.0/7
...
loss = radio_cls_loss*cls_loss_op + radio_bbox_loss*bbox_loss_op + animoji_loss_op*radio_animoji_loss + L2_loss_op
...
try:
    ...
    for step in range(MAX_STEP):
        ...
        if (step+1) % display == 0:
            cls_loss, bbox_loss,animoji_loss,L2_loss,lr,acc,recall = \
                sess.run([cls_loss_op, bbox_loss_op,animoji_loss_op,L2_loss_op,lr_op,accuracy_op,recall_op],
                feed_dict={input_image: image_batch_array, label: label_batch_array, bbox_target: \
                bbox_batch_array,  \
                animoji_target: animoji_batch_array})
            ...
            print('{}: Step: {}, acc: {:.3}, recall: {:.3}, cls loss: {:.4}, bbox loss: {:.4}, animoji loss: {:.4}, L2 loss: {:.4}, lr:{:.4} '.format(
                datetime.now(), step+1, acc, recall, cls_loss, bbox_loss,  animoji_loss, L2_loss, lr))
        ...
        if i * BATCH_SIZE > num:
            ...
            print('{}: {}, Step: {}, acc: {:.3}, recall: {:.3}, cls loss: {:.4}, bbox loss: {:.4},  animoji loss: {:.4}, L2 loss: {:.4}, lr:{:.4} '.format( \
                'Save model epoch', epoch, step+1, acc, recall, cls_loss, bbox_loss, animoji_loss, L2_loss, lr))
            saver.save(sess, prefix, global_step=epoch)
            # saver.save(sess, prefix, global_step=step)
        writer.add_summary(summary,global_step=step)
